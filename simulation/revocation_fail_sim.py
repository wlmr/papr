...
def run(params):
    ...
    logging.basicConfig(format='%(message)s', filename=f'revocation_fail_{k}_{n}_{nbr_of_customers}_sim.log', level=logging.INFO)
    logging.info('total_successful_revocations;total_failed_revocations')
    customers = []
    rev_counter = 0
    rev_fail_counter = 0
    rev_requests = 0
    ...
    while rev_counter + rev_fail_counter < nbr_of_customers:
        ...
        if rev_pub_id is not None:
            ...
            rev_fail_counter += 1
        logging.info(f"{rev_counter};{rev_fail_counter}")


if __name__ == '__main__':
    params = [(3, 10, 100), (9, 10, 100)]
    with Pool() as p:
        p.map(run, params)
