...
def mac(params, sk, m):
    ...
    Hx = sk[0] + sk[1] * em   #x0 + x1*m                  # Hx becomes larger than mod value??!??!
    Hy = sk[2] + sk[3] * em   #y0 + y1*m
...
def verify(params, sk, m, sigma):
    ...
    em = G.encode(m)
    Hx = sk[0] + sk[1]*em   #x0 + x1*m
    Hy = sk[2] + sk[3]*em   #y0 + y1*m
    #Hx = sk[0]   + sum([sk[i+1]*m[i] for i in range(n)])
    #Hy = sk[0+n] + sum([sk[n+i+1]*m[i] for i in range(n)])

    return sigma_w != 1 and sigma_x == sigma_w**Hx and sigma_y == sigma_w**Hy and sigma_z == sigma_w**sk[-1]
