...
from papr.legacy.papr_procedures import req_eq_id, iss_eq_id
from papr.legacy.papr_procedures import req_anon_auth
from papr.legacy.papr_procedures import data_distrubution_issuer_verify, \
    ...
    iss_data_dist_1, iss_data_dist_2, iss_data_dist_3, req_data_dist_1, req_data_dist_2, req_data_dist_3
...
class TestPapr:
    ...
    def test_eq_id(self):
        ...
        (u, cl, _), _, z = req_anon_auth(params, iparams, t_id, priv_id)
        h = p.random() * g0  # making up a random generator (supposed to come from data distribution)
        c0 = priv_id * h
        y, c, gamma = req_eq_id(params, u, h, priv_id, z, cl, c0)
        assert iss_eq_id(params, u, h, y, c, gamma, cl, c0)
    ...
    def test_data_distrubution_2(self):
        ...
        # Both publishes their commits. When they recive the other ones commit they send their random value.
        # and verifyes that the commit and random value they recived are correct.
        assert req_data_dist_2(params, issuer_commit, issuer_random) is True
        custodian_list = iss_data_dist_2(params, requester_commit, requester_random, issuer_random, pub_keys, n)
        assert custodian_list is not None

        E_list, C_list, proof, group_generator = req_data_dist_3(params, requester_random, issuer_random, my_priv_key, pub_keys, k, n)

        # selected_pub_keys = data_distrubution_select(pub_keys, r1, r2, n, p)
        # E_list, C_list, proof, group_generator = data_distrubution_U_2(params, my_priv_key, selected_pub_keys, k, n)

        assert iss_data_dist_3(params, E_list, C_list, proof, custodian_list, group_generator)
